...
| **Guardrail shell-injection guard**   | Guardrail `check.field` is validated to a safe character set (a dotted JSON-key path). It, the condition regexes, and the message are passed as positional arguments to a fixed script, never interpolated onto a command line.                                                                                        |
| **Guardrail regex validation**        | `check.matches` / `check.not_matches` are POSIX ERE compiled by the host libc, not JavaScript regexes. Each is validated as portable ERE at sync time; a pattern that would not compile — or a JS/Perl idiom such as `\d` that compiles to something else — is a sync error and generates no hook.                     |
| **Guardrail conditions fail closed**  | `[[ =~ ]]` returns status 2, not 1, when a pattern fails to compile. The generated hook captures the status explicitly and blocks on anything other than a clean evaluation, so a condition that cannot be evaluated denies the tool call instead of allowing it (GHSA-grpx-fj8v-q8g9).                                |
