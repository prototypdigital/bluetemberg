...
import type { MarketplacePluginDefinition, SyncResults } from '../types.js';
...
function readSkillMeta(skillDir: string, sourceParent: string): { name: string; description: string } {
  ...
  try {
    ...
    return { name: skillDir, description: '' };
  }
}

function readAgentMeta(agentFile: string, sourceDir: string): { name: string; description: string } {
  ...
  try {
    ...
    return { name: basename(agentFile, '.md'), description: '' };
  }
}
